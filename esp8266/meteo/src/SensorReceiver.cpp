...
void SensorReceiver::interruptHandler()
{
	...
	*/
	...
	else
	{
		...
		if (duration < (clockTime >> 1) || duration > (clockTime << 1) + clockTime)
		{ // read as: duration < 0.5 * clockTime || duration > 3 * clockTime
			// Fail. Abort.
			reset();
			return;
		}
		...
		if (halfBit & 1)
		{
			...
			{ // Third byte has been received
				...
				if (packageLength < 5 || packageLength > 11)
				{
					reset();
		...
		// Edge is long?
		if (duration > clockTime + (clockTime >> 1))
